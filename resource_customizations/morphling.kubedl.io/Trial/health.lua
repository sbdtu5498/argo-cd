...
if obj.status ~= nil then
  if obj.status.conditions ~= nil then
    for i, condition in ipairs(obj.status.conditions) do
      if condition. == "Running" and condition.status == "False" then
        hs.status = "Degraded"
      ...
      if condition.type == "Pending" and condition.status == "False" then
        hs.status = "Degraded"
        hs.message = condition.message
        return hs
      end
      if condition.type == "Pending" and condition.status == "True" then
        ...
        hs.message = condition.message
        return hs
      if condition.type == "Killed" and condition.status == "False" then
        hs.status = "Degraded"
        hs.message = condition.message
        return hs
      if condition.type == "Killed" and condition.status == "True" then
        hs.status = "Suspended"
        hs.message = condition.message
        return hs
      end
